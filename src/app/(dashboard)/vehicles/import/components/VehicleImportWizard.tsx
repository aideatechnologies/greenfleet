...
} from "@/lib/services/vehicle-import-service";
import {
  importVehiclesAction,
  getTenantVehiclePlatesForImportAction,
} from "../actions/import-vehicles";
import type { VehicleImportConfig } from "@/lib/schemas/vehicle-import";
import {
  VEHICLE_FIELD_LABELS,
  VEHICLE_IMPORTABLE_FIELDS,
  VEHICLE_REQUIRED_FIELDS,
} from "@/lib/schemas/vehicle-import";
import type { VehicleImportValidation } from "@/lib/schemas/vehicle-import";
...
export function VehicleImportWizard() {
  ...
  const [config, setConfig] = useState<VehicleImportConfig>({
    separator: ";",
    hasHeader: true,
    encoding: "UTF-8",
    numberFormat: "IT",
  ...
  const handleFileSelected = useCallback(
    async (file: File, fileConfig: VehicleImportConfig) => {
      try {
  ...
  // --- Mapping step -> validate + go to preview ---
  const handleMappingNext = useCallback(async () => {
    const missingRequired = VEHICLE_REQUIRED_FIELDS.filter(
      (f) => !(f in mapping)
    );
    if (missingRequired.length > 0) {
      toast.error(
        `Mappa almeno i campi obbligatori: ${missingRequired.map((f) => VEHICLE_FIELD_LABELS[f]).join(", ")}`
      );
      return;
    }
    ...
    try {
      const platesResult = await getTenantVehiclePlatesForImportAction();
      ...
      for (const v of platesResult.data) {
        existingPlates.add(normalizeLicensePlate(v.licensePlate));
      }
  ...
  const handleConfirmImport = useCallback(async () => {
    ...
    try {
      const rows = validResults.map((r) => ({
        ...
        marca: r.resolved!.marca,
        modello: r.resolved!.modello,
        allestimento: r.resolved!.allestimento,
        vin: r.resolved!.vin,
        registrationDate: r.resolved!.registrationDate.toISOString(),
        fuelType: r.resolved!.fuelType,
        status: r.resolved!.status,
      }));

      const result = await importVehiclesAction(rows);
      ...
      if (result.success) {
        ...
        if (result.data.importedRows > 0) {
          toast.success(
            `${result.data.importedRows} ${result.data.importedRows === 1 ? "veicolo importato" : "veicoli importati"} con successo`
          );
        } else {
          toast.warning("Nessun veicolo importato");
        }
      } else {
          ...
          <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setConfig({
                          ...config,
                          encoding: value as VehicleImportConfig["encoding"],
                        })
                      }
                    >
                      <SelectTrigger><SelectValue /></SelectTrigger>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setConfig({ ...config, numberFormat: value as "IT" | "EN" })
                      }
                    >
                      <SelectTrigger><SelectValue /></SelectTrigger>
                      <SelectContent>
                  ...
                  <div className="flex items-center gap-3 pt-6">
                    <Switch
                      ...
                      onCheckedChange={(checked) =>
                        setConfig({ ...config, hasHeader: checked })
                      }
                    />
            ...
            {selectedFile && (
              <div className="flex justify-end">
                <Button onClick={() => handleFileSelected(selectedFile, config)}>
                  Avanti
                </Button>
              </div>
            )}
          ...
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              ...
              <span className="font-medium text-foreground">
                Targa, Marca, Modello, Data Immatricolazione e Tipo Alimentazione
              </span>{" "}
              sono obbligatori.
            </p>
                ...
                <TableBody>
                  {parsedData.headers.map((header, colIndex) => {
                    ...
                    const isRequired =
                      ...
                      VEHICLE_REQUIRED_FIELDS.includes(currentField);
                        ...
                        <TableCell>
                          ...
                          >
                            ...
                            <SelectContent>
                              ...
                              {VEHICLE_IMPORTABLE_FIELDS.map((field) => (
                                <SelectItem key={field} value={field}>
                                  {VEHICLE_FIELD_LABELS[field]}
                                  {VEHICLE_REQUIRED_FIELDS.includes(field) ? " *" : ""}
                                </SelectItem>
                              ))}
                            </SelectContent>
                          </Select>
            ...
            <p className="text-xs text-muted-foreground">
              Totale righe dati nel file: {parsedData.rows.length}
            </p>
          </div>
        )}
                  ...
                  {filteredResults.length === 0 ? (
                    ...
                    filteredResults.map((result) => (
                      ...
                      >
                        ...
                        <TableCell>{result.data.modello || "-"}</TableCell>
                              ...
                              {result.errors.map((err, i) => (
                                <li key={i}>
                                  <span className="font-medium">
                                    {VEHICLE_FIELD_LABELS[err.field] ?? err.field}:
                                  </span>{" "}
                                  {err.message}
                                </li>
                              ))}
          ...
          <div className="space-y-6">
                ...
                {errorRowCount > 0 && (
                  <div className="rounded-md border border-amber-200 bg-amber-50 dark:bg-amber-950/20 dark:border-amber-800 p-3">
                    <p className="text-sm text-amber-800 dark:text-amber-200">
                      ...
                      <span className="font-bold">{validRowCount}</span> righe valide
                      verranno importate.
                    </p>
                  </div>
                )}
            ...
            <div className="flex items-start gap-3">
              ...
              <Label htmlFor="confirm" className="text-sm leading-relaxed cursor-pointer">
                ...
                <span className="font-bold">{validRowCount}</span>{" "}
                {validRowCount === 1 ? "veicolo" : "veicoli"} nel sistema.
              </Label>
            </div>
            ...
            <div className="flex justify-end">
              <Button
                ...
                disabled={!confirmed || isImporting || validRowCount === 0}
                size="lg"
              >
                ...
                <div className="flex flex-col items-center text-center space-y-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-muted-foreground">
                      {importResult.importedRows === 0
                        ? "Nessun veicolo importato."
                        : `${importResult.importedRows} ${importResult.importedRows === 1 ? "veicolo importato" : "veicoli importati"} con successo.`}
                    </p>
                    <p className="mt-1 text-xs text-muted-foreground">
                      Durata: {formatDuration(importResult.durationMs)}
                    </p>
                  </div>
                </div>
      ...
      <AlertDialog open={cancelDialogOpen} onOpenChange={setCancelDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tutti i dati caricati andranno persi. Sei sicuro di voler annullare?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
