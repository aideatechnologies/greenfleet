...
import { updateCarlistSchema } from "@/lib/schemas/carlist";
import { updateCarlist } from "@/lib/services/carlist-service";
...
export async function updateCarlistAction(
  ...
  const parsed = updateCarlistSchema.safeParse(input);
  if (!parsed.success) {
    return {
      success: false,
  ...
  try {
    ...
    const result = await updateCarlist(prisma, id, parsed.data);

    if (result.success) {
      revalidatePath("/carlist");
      revalidatePath(`/carlist/${id}`);
    }
    ...
    logger.error(
      ...
      "Failed to update carlist"
    );
    return {
      success: false,
      error: "Errore nell'aggiornamento della carlist",
