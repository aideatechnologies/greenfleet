...
import {
  ...
  autoMapColumns,
  validateImportRows,
} from "@/lib/services/km-reading-import-service";
import {
  importKmReadingsAction,
  getKmReadingImportLookupsAction,
} from "../actions/import-km-readings";
import type { KmReadingImportConfig } from "@/lib/schemas/km-reading-import";
import {
  KM_READING_FIELD_LABELS,
  KM_READING_IMPORTABLE_FIELDS,
  KM_READING_REQUIRED_FIELDS,
} from "@/lib/schemas/km-reading-import";
import type { KmReadingImportValidation } from "@/lib/schemas/km-reading-import";
...
export function KmReadingImportWizard() {
  const router = useRouter();

  const [currentStep, setCurrentStep] = useState<StepKey>("upload");
  const [cancelDialogOpen, setCancelDialogOpen] = useState(false);

  const [config, setConfig] = useState<KmReadingImportConfig>({
    separator: ";",
    hasHeader: true,
    encoding: "UTF-8",
    numberFormat: "IT",
  ...
  const [importResult, setImportResult] =
    useState<KmReadingImportResult | null>(null);
  ...
  const handleFileSelected = useCallback(async (file: File, fileConfig: KmReadingImportConfig) => {
    try {
  ...
  const handleMappingNext = useCallback(async () => {
    const missingRequired = KM_READING_REQUIRED_FIELDS.filter((f) => !(f in mapping));
    if (missingRequired.length > 0) {
      toast.error(`Mappa almeno i campi obbligatori: ${missingRequired.map((f) => KM_READING_FIELD_LABELS[f]).join(", ")}`);
      return;
    }
    ...
    try {
      const lookupsResult = await getKmReadingImportLookupsAction();
      ...
      for (const v of lookupsResult.data) {
  ...
  const handleConfirmImport = useCallback(async () => {
    ...
    try {
      ...
      const result = await importKmReadingsAction(rows);
      if (result.success) {
        ...
        if (result.data.importedRows > 0) {
          toast.success(`${result.data.importedRows} ${result.data.importedRows === 1 ? "rilevazione importata" : "rilevazioni importate"} con successo`);
        } else {
          toast.warning("Nessuna rilevazione importata");
        }
      } else {
  ...
  const handleConfirmCancel = useCallback(() => { setCancelDialogOpen(false); router.push("/km-readings"); }, [router]);
    ...
    <div className="space-y-6">
      ...
      <div className="min-h-[300px]">
            ...
            {selectedFile && isCSV && (
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                  ...
                  <div className="space-y-2"><Label>Codifica</Label><Select value={config.encoding} onValueChange={(v) => setConfig({ ...config, encoding: v as KmReadingImportConfig["encoding"] })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="UTF-8">UTF-8</SelectItem><SelectItem value="ISO-8859-1">Latin-1</SelectItem><SelectItem value="Windows-1252">Windows-1252</SelectItem></SelectContent></Select></div>
        ...
        {currentStep === "mapping" && parsedData && (
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">Associa le colonne del file ai campi. <span className="font-medium text-foreground">Targa, Data Rilevamento e Km Rilevati</span> sono obbligatori.</p>
            <div className="rounded-md border"><Table><TableHeader><TableRow><TableHead className="w-[200px]">Colonna file</TableHead><TableHead className="w-[200px]">Valore anteprima</TableHead><TableHead className="w-[250px]">Campo Greenfleet</TableHead></TableRow></TableHeader><TableBody>
              {parsedData.headers.map((header, colIndex) => {
                ...
                const isRequired = currentField !== UNMAPPED_VALUE && KM_READING_REQUIRED_FIELDS.includes(currentField);
                return (
        ...
        {currentStep === "preview" && (
          <div className="space-y-4">
            ...
            <div className="rounded-md border overflow-auto max-h-[500px]"><Table><TableHeader><TableRow><TableHead className="w-[60px] text-center">Riga</TableHead><TableHead className="w-[80px]">Stato</TableHead><TableHead>Targa</TableHead><TableHead>Data</TableHead><TableHead>Km</TableHead><TableHead>Dettagli</TableHead></TableRow></TableHeader><TableBody>
              {filteredResults.length === 0 ? (
                ...
                <TableRow key={result.rowIndex} className={!result.isValid ? "bg-red-50/50 dark:bg-red-950/10" : result.warnings.length > 0 ? "bg-amber-50/50 dark:bg-amber-950/10" : ""}>
                  ...
                  <TableCell>
                    {result.errors.length > 0 ? (<ul className="list-disc pl-4 text-xs text-red-600 dark:text-red-400 space-y-0.5">{result.errors.map((err, i) => (<li key={i}><span className="font-medium">{KM_READING_FIELD_LABELS[err.field] ?? err.field}:</span> {err.message}</li>))}</ul>)
        ...
        {currentStep === "result" && importResult && (
          <div className="space-y-6">
            <Card><CardContent className="pt-6">
              <div className="flex flex-col items-center text-center space-y-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-muted-foreground">{importResult.importedRows === 0 ? "Nessuna rilevazione importata." : `${importResult.importedRows} ${importResult.importedRows === 1 ? "rilevazione importata" : "rilevazioni importate"} con successo.`}</p>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:justify-center">
              <Button asChild><Link href="/km-readings"><Gauge className="mr-2 h-4 w-4" />Vai alla lista rilevazioni</Link></Button>
