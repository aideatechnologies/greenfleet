...
export function ImportResult({ summary, onReset }: ImportResultProps) {
  const hasErrors = summary.errorRows > 0 || summary.skippedRows > 0;
  const allFailed = summary.importedRows === 0;
          ...
          <div className="flex flex-col items-center text-center space-y-4">
            ...
            <div>
              ...
              <p className="mt-1 text-muted-foreground">
                {allFailed
                  ? "Nessun dipendente è stato importato."
                  : `${summary.importedRows} ${summary.importedRows === 1 ? "dipendente importato" : "dipendenti importati"} con successo.`}
              </p>
            </div>
          </div>
