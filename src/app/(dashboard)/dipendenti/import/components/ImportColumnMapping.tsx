...
export function ImportColumnMapping({
  ...
  // Fields already mapped (to prevent duplicates)
  const mappedFields = useMemo(() => new Set(Object.keys(mapping)), [mapping]);
    ...
    <div className="space-y-4">
      <div>
        <p className="text-sm text-muted-foreground">
          ...
          <span className="font-medium text-foreground">Cognome</span> sono
          obbligatori.
        </p>
      </div>
            ...
            {headers.map((header, colIndex) => {
              ...
              const isRequired =
                ...
                REQUIRED_FIELDS.includes(
                  currentField as (typeof REQUIRED_FIELDS)[number]
                );
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        {IMPORTABLE_FIELDS.map((field) => {
                          // Allow the field if it's currently mapped to this column
                          // or if it's not mapped to any column
                          const canSelect =
                            currentField === field || !usedIndices.has(mapping[field]);
                          const isMapped =
