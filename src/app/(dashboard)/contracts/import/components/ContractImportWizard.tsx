...
} from "@/lib/services/contract-import-service";
import {
  importContractsAction,
  getContractImportLookupsAction,
} from "../actions/import-contracts";
import type { ContractImportConfig } from "@/lib/schemas/contract-import";
import {
  CONTRACT_FIELD_LABELS,
  CONTRACT_IMPORTABLE_FIELDS,
  CONTRACT_REQUIRED_FIELDS,
} from "@/lib/schemas/contract-import";
import type { ContractImportValidation } from "@/lib/schemas/contract-import";
...
export function ContractImportWizard() {
  ...
  const [config, setConfig] = useState<ContractImportConfig>({
    separator: ";",
    hasHeader: true,
    encoding: "UTF-8",
    numberFormat: "IT",
  ...
  const handleFileSelected = useCallback(
    async (file: File, fileConfig: ContractImportConfig) => {
      try {
  ...
  const handleMappingNext = useCallback(async () => {
    const missingRequired = CONTRACT_REQUIRED_FIELDS.filter(
      (f) => !(f in mapping)
    );
    if (missingRequired.length > 0) {
      toast.error(
        `Mappa almeno i campi obbligatori: ${missingRequired.map((f) => CONTRACT_FIELD_LABELS[f]).join(", ")}`
      );
      return;
    }
    ...
    try {
      const lookupsResult = await getContractImportLookupsAction();
      if (!lookupsResult.success) {
        toast.error(lookupsResult.error);
  ...
  const handleConfirmImport = useCallback(async () => {
    ...
    try {
      const rows = validResults.map((r) => ({
        contractNumber: r.resolved!.contractNumber,
        vehicleId: r.resolved!.vehicleId,
        licensePlate: r.resolved!.licensePlate,
        contractType: r.resolved!.contractType,
        supplierId: r.resolved!.supplierId,
        supplierName: r.resolved!.supplierName,
        startDate: r.resolved!.startDate.toISOString(),
        endDate: r.resolved!.endDate ? r.resolved!.endDate.toISOString() : null,
        monthlyRate: r.resolved!.monthlyRate,
        franchiseKm: r.resolved!.franchiseKm,
      }));

      const result = await importContractsAction(rows);
      ...
      if (result.success) {
        ...
        if (result.data.importedRows > 0) {
          toast.success(
            `${result.data.importedRows} ${result.data.importedRows === 1 ? "contratto importato" : "contratti importati"} con successo`
          );
        } else {
          toast.warning("Nessun contratto importato");
        }
      } else {
  ...
  const handleFieldChange = useCallback(
    (colIndex: number, fieldName: string) => {
      ...
      for (const [field, idx] of Object.entries(newMapping)) {
        if (idx === colIndex) delete newMapping[field];
      }
        ...
        {STEPS.map((step, index) => {
          ...
          return (
            <div key={step.key} className="flex items-center gap-2">
              {index > 0 && (
                <div className={`h-[2px] w-8 ${isCompleted ? "bg-primary" : "bg-muted"}`} />
              )}
              <div
          ...
          <div className="space-y-6">
            ...
            {selectedFile && (
              <Card>
                <CardContent className="flex items-center gap-3 py-3">
                  ...
                  <Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); setSelectedFile(null); setFileError(null); if (inputRef.current) inputRef.current.value = ""; }}>
                    <X className="h-4 w-4" />
                  </Button>
                </CardContent>
              </Card>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={config.encoding} onValueChange={(v) => setConfig({ ...config, encoding: v as ContractImportConfig["encoding"] })}>
                      <SelectTrigger><SelectValue /></SelectTrigger>
                      <SelectContent>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={config.numberFormat} onValueChange={(v) => setConfig({ ...config, numberFormat: v as "IT" | "EN" })}>
                      <SelectTrigger><SelectValue /></SelectTrigger>
                      <SelectContent>
                  ...
                  <div className="flex items-center gap-3 pt-6">
                    <Switch id="hasHeader" checked={config.hasHeader} onCheckedChange={(checked) => setConfig({ ...config, hasHeader: checked })} />
                    <Label htmlFor="hasHeader">Prima riga = intestazioni</Label>
                  </div>
                </div>
              </div>
            )}
            {selectedFile && (
              <div className="flex justify-end">
                <Button onClick={() => handleFileSelected(selectedFile, config)}>Avanti</Button>
              </div>
            )}
          </div>
        )}
        ...
        {currentStep === "mapping" && parsedData && (
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              ...
              <span className="font-medium text-foreground">Numero Contratto, Targa, Tipo Contratto e Data Inizio</span>{" "}
              sono obbligatori.
            </p>
              ...
              <Table>
                ...
                <TableBody>
                  {parsedData.headers.map((header, colIndex) => {
                    ...
                    const isRequired = currentField !== UNMAPPED_VALUE && CONTRACT_REQUIRED_FIELDS.includes(currentField);
                    return (
                      <TableRow key={colIndex}>
                        <TableCell className="font-medium">
                          {header}
                          {isRequired && <Badge variant="secondary" className="ml-2 text-xs">Obbligatorio</Badge>}
                        </TableCell>
                        <TableCell className="text-muted-foreground">
                          {parsedData.rows[0]?.[colIndex] ?? <span className="italic">vuoto</span>}
                        </TableCell>
                        <TableCell>
                          <Select value={currentField} onValueChange={(v) => handleFieldChange(colIndex, v)}>
                            <SelectTrigger className="w-[220px]"><SelectValue placeholder="Non mappato" /></SelectTrigger>
                            <SelectContent>
                              <SelectItem value={UNMAPPED_VALUE}>-- Non mappato --</SelectItem>
                              {CONTRACT_IMPORTABLE_FIELDS.map((field) => (
                                <SelectItem key={field} value={field}>
                                  {CONTRACT_FIELD_LABELS[field]}{CONTRACT_REQUIRED_FIELDS.includes(field) ? " *" : ""}
                                </SelectItem>
                              ))}
                            </SelectContent>
                          </Select>
          ...
          <div className="space-y-4">
            ...
            {errorRowCount > 0 && (
              <div className="flex items-center gap-3">
                <Switch id="showOnlyErrors" checked={showOnlyErrors} onCheckedChange={setShowOnlyErrors} />
                  ...
                  {filteredResults.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={6} className="h-24 text-center text-muted-foreground">
                        {showOnlyErrors ? "Nessuna riga con errori" : "Nessun risultato"}
                      </TableCell>
                    </TableRow>
                  ) : (
                    filteredResults.map((result) => (
                      <TableRow key={result.rowIndex} className={!result.isValid ? "bg-red-50/50 dark:bg-red-950/10" : result.warnings.length > 0 ? "bg-amber-50/50 dark:bg-amber-950/10" : ""}>
                        ...
                        <TableCell>
                          <Badge variant={result.isValid ? "default" : "destructive"} className={result.isValid ? (result.warnings.length > 0 ? "bg-amber-600 hover:bg-amber-600/90" : "bg-green-600 hover:bg-green-600/90") : ""}>
                            {result.isValid ? (result.warnings.length > 0 ? "Avviso" : "OK") : "Errore"}
                          </Badge>
                        </TableCell>
                        <TableCell>{result.data.contractNumber || "-"}</TableCell>
                        <TableCell><code className="rounded bg-muted px-1.5 py-0.5 text-xs font-mono">{result.data.licensePlate || "-"}</code></TableCell>
                        <TableCell>{result.data.contractType || "-"}</TableCell>
                        <TableCell>
                          {result.errors.length > 0 ? (
                            <ul className="list-disc pl-4 text-xs text-red-600 dark:text-red-400 space-y-0.5">
                              {result.errors.map((err, i) => (
                                <li key={i}><span className="font-medium">{CONTRACT_FIELD_LABELS[err.field] ?? err.field}:</span> {err.message}</li>
                              ))}
                            </ul>
        ...
        {currentStep === "confirm" && (
          <div className="space-y-6">
              ...
              <CardContent className="space-y-4 pt-6">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                  ...
                  <div className="rounded-md border border-green-200 dark:border-green-800 px-4 py-3 text-center">
                    ...
                    <p className="text-3xl font-bold text-green-600">{validRowCount}</p>
                  </div>
                  <div className="rounded-md border border-red-200 dark:border-red-800 px-4 py-3 text-center">
                    <div className="flex items-center justify-center gap-2 mb-1">
                      <AlertCircle className="h-4 w-4 text-red-600" />
                      <p className="text-xs text-muted-foreground">Da escludere</p>
                    </div>
                    <p className="text-3xl font-bold text-red-600">{errorRowCount}</p>
                  </div>
                </div>
                {errorRowCount > 0 && (
                  <div className="rounded-md border border-amber-200 bg-amber-50 dark:bg-amber-950/20 dark:border-amber-800 p-3">
                    <p className="text-sm text-amber-800 dark:text-amber-200">
                      Le righe con errori verranno ignorate. Solo le <span className="font-bold">{validRowCount}</span> righe valide verranno importate.
                    </p>
                  </div>
                )}
              </CardContent>
            ...
            <div className="flex items-start gap-3">
              <Checkbox id="confirm" checked={confirmed} onCheckedChange={(checked) => setConfirmed(checked === true)} disabled={isImporting} />
              <Label htmlFor="confirm" className="text-sm leading-relaxed cursor-pointer">
                Confermo di voler importare <span className="font-bold">{validRowCount}</span>{" "}
                {validRowCount === 1 ? "contratto" : "contratti"} nel sistema.
              </Label>
            </div>
            <div className="flex justify-end">
              <Button onClick={handleConfirmImport} disabled={!confirmed || isImporting || validRowCount === 0} size="lg">
        ...
        {currentStep === "result" && importResult && (
          <div className="space-y-6">
            <Card>
              <CardContent className="pt-6">
                <div className="flex flex-col items-center text-center space-y-4">
                  ...
                  ) : importResult.errorRows > 0 || importResult.skippedRows > 0 ? (
                  ...
                  <div>
                    <h3 className="text-xl font-semibold">
                      {importResult.importedRows === 0 ? "Importazione fallita" : importResult.errorRows > 0 ? "Importazione completata con avvisi" : "Importazione completata"}
                    </h3>
                    <p className="mt-1 text-muted-foreground">
                      {importResult.importedRows === 0 ? "Nessun contratto importato." : `${importResult.importedRows} ${importResult.importedRows === 1 ? "contratto importato" : "contratti importati"} con successo.`}
                    </p>
                ...
                <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
                  ...
                  <div className="rounded-md border border-green-200 dark:border-green-800 px-4 py-3 text-center">
                    ...
                    <p className="text-xl font-bold text-green-600">{importResult.importedRows}</p>
                  </div>
                  <div className="rounded-md border border-amber-200 dark:border-amber-800 px-4 py-3 text-center">
                    ...
                    <p className="text-xl font-bold text-amber-600">{importResult.skippedRows}</p>
                  </div>
                  <div className="rounded-md border border-red-200 dark:border-red-800 px-4 py-3 text-center">
                    <p className="text-xs text-muted-foreground">Errori</p>
                    <p className="text-xl font-bold text-red-600">{importResult.errorRows}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            <div className="flex flex-col gap-3 sm:flex-row sm:justify-center">
              <Button asChild>
                <Link href="/contracts"><FileSignature className="mr-2 h-4 w-4" />Vai alla lista contratti</Link>
              </Button>
      ...
      {currentStep !== "upload" && currentStep !== "result" && currentStep !== "confirm" && (
        <div className="flex items-center justify-between border-t pt-4">
          ...
          <div className="flex gap-3">
            ...
            {currentStep === "mapping" && (
              <Button onClick={handleMappingNext} disabled={isValidating}>
                {isValidating ? "Validazione..." : "Avanti"}
              </Button>
            )}
            {currentStep === "preview" && (
              <Button onClick={goNext} disabled={validRowCount === 0}>Avanti</Button>
            )}
          </div>
        </div>
      )}
      ...
      <AlertDialog open={cancelDialogOpen} onOpenChange={setCancelDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Tutti i dati caricati andranno persi. Sei sicuro di voler annullare?</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Continua importazione</AlertDialogCancel>
            <AlertDialogAction onClick={handleConfirmCancel}>Annulla importazione</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
